...
export const CourseDetails = () => {
    ...
    const progress = getCourseProgress(user.id, courseId, modules.length);
    const allComponentsCompleted = hasCompletedCourseComponents(user.id, courseId, modules.length);
    const hasPassedCourse = isCoursePassed(user.id, courseId, modules.length);
    ...
    useEffect(() => {
        ...
        const watchTimer = setTimeout(() => {
            markModuleCompleted(user.id, courseId, currentModule.id);
        }, 15000);

        return () => clearTimeout(watchTimer);
    }, [currentModule?.id, completedModules, markModuleCompleted, user.id, courseId]);
    ...
    const handleAssignmentSubmit = () => {
        ...
        reader.onload = () => {
            submitCourseAssignment(user.id, courseId, {
